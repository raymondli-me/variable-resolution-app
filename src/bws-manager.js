...
function setupBWSEventListeners() {
  ...
  if (includePDFs) {
    includePDFs.addEventListener('change', updateBWSCollectionCount);
  }
...
async function createBWSExperiment() {
  ...
  try {
    ...
    if (bwsState.sourceType === 'rating-project') {
      ...
      config.include_pdfs = bwsState.includePDFs;
    }
...
function renderBWSItemCard(item, index, aiJudgment, humanJudgment) {
  ...
  const content = isComment ? item.text : (isPDF ? item.text_content : item.transcript_text);
  const typeLabel = isComment ? 'Comment' : (isPDF ? 'PDF Excerpt' : 'Video Chunk');
  ...
  card.innerHTML = `
    ...
    ` : `
      <!-- Text Content (for comments and PDFs) -->
    ...
    ` : isPDF ? `
      <div class="bws-item-meta">
        <span>📄 ${item.pdf_title || 'PDF Document'}</span>
        ${item.page_number ? `<span>📖 Page ${item.page_number}</span>` : ''}
        <span>📊 Score: ${(item.relevance_score || item.parent_rating_score || 0).toFixed(2)}</span>
      </div>
    ` : `
      <div class="bws-item-meta">
