...
async function displayCollections(collections) {
  ...
  // Check for incomplete collections to get their status
  const incompleteResult = await window.api.collections.checkIncomplete();
  const incompleteMap = new Map();
  if (incompleteResult?.success && incompleteResult?.incomplete) {
    incompleteResult.incomplete.forEach(item => {
      // Match by collection ID if available, or by search term and rough timestamp
      ...
      <div class="collection-header">
        ...
        <div class="collection-actions">
          ${!collection.isMerge && isIncomplete ? `
            ...
            <button class="btn btn-sm" onclick="markCollectionComplete(${collection.id}, '${incompleteInfo.folderPath}/collection_manifest.json', event);" title="Mark as Complete">
              <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
                <polyline points="20 6 9 17 4 12"></polyline>
              </svg>
            </button>
          ` : ''}
          <button class="btn btn-sm" onclick="exportCollection(${collection.id}, event);" title="Export">
            <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
